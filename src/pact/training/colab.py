...
def _restore_snapshot(snapshot: Path, destination: Path, *, progress=lambda message: None, kind="warmstart"):
    ...
    entries = index["files"]
    if index.get("schema_version") != 1 or not isinstance(entries, dict) or not 1 <= len(entries) <= (40000 if kind in ("controlled_specialization_fixed_bank_v1", "specialization_fixed_bank") else 10000 if kind in ("receiver_supervision", "gpqa_support") else 5000):
        raise ValueError("Invalid snapshot inventory")
    if kind not in ("controlled_specialization_fixed_bank_v1", "specialization_fixed_bank", "gpqa_support", "receiver_supervision", "warmstart", "training_bank", "preference_feasibility_diagnostic", "receiver_feasibility_diagnostic", "private_support_control", "curated_repair_diagnostic", "actor_preparation_probe", "preparation_prompt_control"):
    ...
    if kind in ("controlled_specialization_fixed_bank_v1", "specialization_fixed_bank", "receiver_supervision", "gpqa_support"): required = ({"state.json", "plan.json"} if kind in ("gpqa_support", "specialization_fixed_bank", "controlled_specialization_fixed_bank_v1") else {"state.json", "plan.json", "recipe.json"})
    ...
    if kind in ("controlled_specialization_fixed_bank_v1", "specialization_fixed_bank", "receiver_supervision", "gpqa_support") and snapshot.name != max(p.name for p in snapshot.parent.iterdir() if p.is_dir()):
    ...
    try:
        ...
        if kind == "controlled_specialization_fixed_bank_v1":
            from .controlled_specialization import load_plan, validate_recovery
            plan = load_plan(staging)
            if not read_json(staging / "state.json")["recovery_safe"]:
                raise ValueError("Unsafe controlled snapshot; no rollback")
            validate_recovery(staging, plan)
        elif kind == "specialization_fixed_bank":
