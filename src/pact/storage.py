...
def storage_operation(operation: str, source: Path, destination: Path, *,
    ...
    if operation not in ("snapshot", "bundle", "bundle-restore", "warmstart-restore", "collection-restore", "feasibility-restore", "receiver-restore", "private-control-restore", "curated-repair-restore") or not 0 < timeout_seconds < float("inf"):
...
def _worker(control: Path) -> int:
    ...
    try:
        if request["operation"] in ("warmstart-restore", "collection-restore", "feasibility-restore", "receiver-restore", "private-control-restore", "curated-repair-restore"):
            ...
            kind = {"warmstart-restore":"warmstart", "collection-restore":"training_bank",
                    ...
                    "curated-repair-restore":"curated_repair_diagnostic"}[request["operation"]]
