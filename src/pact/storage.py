...
def storage_operation(operation: str, source: Path, destination: Path, *,
    ...
    if operation not in ("snapshot", "bundle", "bundle-restore", "warmstart-restore", "collection-restore", "feasibility-restore", "receiver-restore") or not 0 < timeout_seconds < float("inf"):
...
def _worker(control: Path) -> int:
    ...
    try:
        if request["operation"] in ("warmstart-restore", "collection-restore", "feasibility-restore", "receiver-restore"):
            ...
            kind = {"warmstart-restore":"warmstart", "collection-restore":"training_bank",
                    ...
                    "receiver-restore":"receiver_feasibility_diagnostic"}[request["operation"]]
